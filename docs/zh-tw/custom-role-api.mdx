---
title: 憑證管理員角色 - API
description: 透過 API 建立自訂角色，限制使用者僅能管理 SSL/TLS 憑證
sidebar:
  label: 自訂角色 - API
i18n:
...
本指南將逐步說明如何在 F5 Distributed Cloud 中建立自訂 RBAC 角色，
授予管理 HTTP Load Balancer 所使用的 SSL/TLS 憑證和憑證鏈的權限。若需要透過 UI 操作的方式，請參閱
[簡化主控台指南](./custom-role-ui)。
...
api_group_element  (唯讀，系統定義)
   ...
   api_group       (唯讀，系統定義)
       │
       ▼
     role           (完整 CRUD — 這是您需要建立的)
```

1. **`api_group_element`** — 定義正規表達式路徑和 HTTP 方法
   （例如 `POST /api/config/.*/certificates`）。由系統管理；您無法
   建立、修改或刪除這些項目。

2. **`api_group`** — 按功能區域組織的 `api_group_element` 參照之具名集合。由系統管理；您無法建立、修改或
   刪除這些項目。群組使用如 `ves-io-proxy-read` 或
   `f5xc-waap-standard-admin` 的命名慣例。

3. **`role`** — 在陣列中參照 `api_group` 名稱。**這是您唯一需要建立的層級。**使用自訂角色端點將特定的
   `api_groups` 附加到角色上。

HTTP Load Balancer 的憑證操作與其他代理相關資源（HTTP Load Balancer、路由、來源等）一起被歸入
`ves-io-proxy-*` api_groups 中。沒有獨立的憑證專用
api_groups — 平台按功能區域而非個別資源類型來組織權限。

## 範圍與限制

`ves-io-proxy-read` 和 `ves-io-proxy-write` api_groups 授予對
**所有代理相關資源**的存取權限，而不僅僅是憑證。被指派此角色的使用者也可以管理 HTTP Load Balancer、來源、路由和其他
代理範圍內的物件。

在 F5 XC 上無法實現僅限憑證的 RBAC。`api_group` 和
`api_group_element` 都是系統定義的唯讀物件 — API 僅為這些物件公開 GET 端點。角色只能參照現有的 `api_group`
名稱，且不存在專門限定於憑證的 `api_group`。

**緩解措施 — 命名空間隔離：**建立專用命名空間來存放
憑證資源，並將 `example-cert-admin` 角色僅指派到該命名空間的範圍內。使用者保留代理層級的權限，但無法存取
其他命名空間中的資源，從而限制較廣泛 api_group 範圍的影響範圍。
...
列出所有系統定義的 `api_group` 物件，並篩選代理相關的
項目（其中包含憑證權限）：
...
| API 群組 | 授予的權限 |
| --------- | ------ |
| `ves-io-proxy-read` | 對代理資源（包括憑證）的讀取存取權限 |
| `ves-io-proxy-write` | 對代理資源（包括憑證）的寫入存取權限 |

:::note
這些群組涵蓋所有代理相關資源（HTTP Load Balancer、來源、
路由、憑證、憑證鏈等），而不僅僅是憑證。F5 XC
按功能區域組織權限。如果您需要驗證某個群組授予了哪些特定的
API 路徑，請查看 API 參考章節中描述的 `api_group_elements` 端點。
:::

## 步驟 2：建立自訂角色

使用自訂角色建立端點。`api_groups` 陣列參照在步驟 1 中發現的
系統定義 api_group 名稱。

建立具有代理資源（包括憑證和憑證鏈）讀寫存取權限的自訂角色：
...
使用自訂 GET 端點擷取角色，以確認 `api_groups`
已附加：
...
若要在現有角色中新增或移除 `api_groups`，請使用自訂 PUT
端點：
...
這些物件是由 F5 XC **系統定義**的。平台會為每個 API 路徑預先建立
`api_group_element` 項目，並將它們按功能區域分組到
`api_group` 物件中。

您不需要建立這些項目 — 它們已經為系統中的每個資源存在。您的任務是**探索**相關的群組名稱（步驟 1）並在您的自訂角色中**參照**它們（步驟 2）。

若要查看某個 api_group 涵蓋了哪些個別 API 路徑，請列出
`api_group_elements` 端點並尋找與您的資源相關的元素：

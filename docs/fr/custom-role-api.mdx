...
Ce guide vous accompagne dans la création d'un rôle RBAC personnalisé dans F5 Distributed Cloud
...
[guide simplifié de la console](./custom-role-ui).
...
1. **`api_group_element`** — Définit un chemin regex et des méthodes HTTP
   (par ex., `POST /api/config/.*/certificates`). Géré par le système ; vous ne pouvez pas
   les créer, modifier ou supprimer.
...
rôle peut également gérer les HTTP Load Balancers, les origines, les routes et autres
objets relevant du scope proxy.

Un RBAC limité aux seuls certificats n'est pas possible sur F5 XC. Les objets `api_group` et
`api_group_element` sont tous deux définis par le système et en lecture seule — l'API n'expose que des endpoints GET pour ceux-ci. Les rôles ne peuvent référencer que des noms d'`api_group` existants, et il n'existe aucun `api_group` limité exclusivement aux certificats.
...
## Étape 1 : Découvrir les groupes d'API proxy

Listez tous les objets `api_group` définis par le système et filtrez les
entrées liées au proxy (qui incluent les permissions sur les certificats) :
...
d'API spécifiques un groupe accorde, consultez l'endpoint `api_group_elements` décrit dans
la section Référence de l'API.
:::
...
Créez le rôle personnalisé avec un accès en lecture et écriture aux ressources proxy (incluant
...
Cet exemple rétrograde le rôle à un accès en lecture seule en supprimant
`ves-io-proxy-write`.
...
### Endpoints de rôle (personnalisés — incluant les `api_groups`)
...
Pour voir quels chemins d'API individuels un api_group couvre, listez l'endpoint

---
title: Role de Administrador de Certificados - API
description: >-
  Criar um role personalizado restringindo usuários ao gerenciamento de
  certificados SSL/TLS via API
sidebar:
  label: Role Personalizado - API
i18n:
  sourceHash: a14b5afb38e8
  translator: machine
---

Este guia orienta a criação de um role RBAC personalizado no F5 Distributed Cloud
que concede permissão para gerenciar certificados SSL/TLS e cadeias de certificados
utilizados por HTTP Load Balancers. Para uma abordagem baseada em interface, consulte o
[guia simplificado do console](/administration/pt-br/custom-role-ui/).
...
1. **`api_group_element`** — Define um caminho regex e métodos HTTP
   ...
   criar, modificar ou excluir esses objetos.
...
2. **`api_group`** — Uma coleção nomeada de referências a `api_group_element`,
   ...
   excluir esses objetos. Os grupos usam convenções de nomenclatura como `ves-io-proxy-read` ou
   `f5xc-waap-standard-admin`.

3. **`role`** — Referencia nomes de `api_group` em um array. **Esta é a única
   camada que você cria.** Use os endpoints de role personalizado para anexar
   `api_groups` específicos a um role.
...
organiza permissões por área funcional. Se você precisar verificar quais caminhos de API
específicos um grupo concede, consulte o endpoint `api_group_elements` descrito na
seção Referência da API.
:::

## Etapa 2: Criar o Role Personalizado

Use o endpoint de criação de role personalizado. O array `api_groups` referencia os
nomes de api_group definidos pelo sistema descobertos na Etapa 1.

Crie o role personalizado com acesso de leitura e escrita a recursos de proxy (incluindo
...
## Etapa 3: Verificar o Role

Recupere o role usando o endpoint GET personalizado para confirmar que os `api_groups`
estão anexados:
...
## Etapa 4: Atualizar o Role (Opcional)

Para adicionar ou remover `api_groups` de um role existente, use o endpoint PUT
personalizado:
...
Este exemplo rebaixa o role para acesso somente leitura removendo
`ves-io-proxy-write`.
...
Você não precisa criá-los — eles já existem para cada recurso no
sistema. Sua tarefa é **descobrir** os nomes de grupo relevantes (Etapa 1) e
**referenciá-los** no seu role personalizado (Etapa 2).

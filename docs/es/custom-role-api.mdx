...
Esta guía le muestra cómo crear un rol RBAC personalizado en F5 Distributed Cloud
que otorga permisos para gestionar certificados SSL/TLS y cadenas de certificados
utilizados por los HTTP Load Balancers. Para un enfoque basado en la interfaz de usuario, consulte la
[guía simplificada de la consola](./custom-role-ui).
...
1. **`api_group_element`** — Define una ruta regex y métodos HTTP
   (ej., `POST /api/config/.*/certificates`). Gestionado por el sistema; no puede
   crear, modificar ni eliminar estos.
...
2. **`api_group`** — Una colección nombrada de referencias a `api_group_element`,
   ...
   eliminar estos. Los grupos usan convenciones de nomenclatura como `ves-io-proxy-read` o
   `f5xc-waap-standard-admin`.

3. **`role`** — Referencia nombres de `api_group` en un array. **Este es el único
   nivel que usted crea.** Use los endpoints de roles personalizados para adjuntar
...
**Mitigación — aislamiento por namespace:** Cree un namespace dedicado para
los recursos de certificados y asigne el rol `example-cert-admin` con alcance únicamente a ese
...
Liste todos los objetos `api_group` definidos por el sistema y filtre por las
...
Use el endpoint de creación de roles personalizados. El array `api_groups` referencia los
...
Recupere el rol usando el endpoint GET personalizado para confirmar que los `api_groups`
están adjuntos:
...
Para agregar o eliminar `api_groups` de un rol existente, use el endpoint PUT
personalizado:
...
Este ejemplo degrada el rol a acceso de solo lectura eliminando
`ves-io-proxy-write`.

## Paso 5: Eliminar el Rol (Opcional)

La eliminación usa el endpoint estándar de roles:
...
El descubrimiento usa el namespace `shared`.

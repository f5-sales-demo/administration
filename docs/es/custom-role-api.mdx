---
title: Rol de Administración de Certificados - API
description: >-
...
Esta guía explica paso a paso cómo crear un rol RBAC personalizado en F5 Distributed Cloud
...
objetos dentro del alcance de proxy.

El RBAC exclusivo para certificados no es posible en F5 XC. Tanto `api_group` como
`api_group_element` son objetos definidos por el sistema y de solo lectura — la API solo
expone endpoints GET para estos. Los roles solo pueden hacer referencia a nombres de `api_group`
existentes, y no existe un `api_group` con alcance exclusivo para certificados.

**Mitigación — aislamiento por namespace:** Cree un namespace dedicado para los
recursos de certificados y asigne el rol `example-cert-admin` con alcance limitado
únicamente a ese namespace. El usuario conserva los permisos a nivel de proxy pero no puede acceder
a recursos en otros namespaces, lo que limita el radio de impacto del alcance
más amplio del api_group.
...
organiza los permisos por área funcional. Si necesita verificar qué rutas de API
...
Utilice el endpoint de creación de roles personalizados. El array `api_groups` hace referencia a los
...
Estos objetos están **definidos por el sistema** por F5 XC. La plataforma pre-crea
